...
class Transform : public Component {
  ...
  public:
    ...
    void update() override {
        float tmpXpos, tmpYpos;
        if (this->entity_ && this->entity_->hasComponent<Input>()) {
                ...
                tmpXpos = this->position_.getX() + (entity_velocity.getX() *
                                                          entity_speed);
                tmpYpos = this->position_.getY() + (entity_velocity.getY() *
                                                          entity_speed);
        } else {
            tmpXpos = this->position_.getX();
            tmpYpos = this->position_.getY();
        }
    ...
    void setDestRect(const int &w, const int &h, const int &x = 0, const int &y = 0) {
        this->dest_.w = w;
        this->dest_.h = h;
        this->dest_.x = x;
        this->dest_.y = y;
    ...
    void setSrcRect(const int &w, const int &h, const int &x = 0, const int &y = 0) {
        this->src_.w = w;
        this->src_.h = h;
        this->src_.x = x;
        this->src_.y = y;
