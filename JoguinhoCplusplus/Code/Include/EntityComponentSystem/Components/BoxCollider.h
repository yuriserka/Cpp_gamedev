...
public:
    ...
    void init() override {
        if (this->entity_) {
            ...
            if (this->entity_->hasComponent<Border>()) {
                auto border_transform = this->entity_->getComponent<Border>()->getTransform();
                this->setColliderDimension(border_transform->getwidth(),
                                           border_transform->getHeight(),
    ...
    void update() override {
        ...
        if (this->entity_) {
            if (this->entity_->hasComponent<Border>() && this->tag_ == "player") {
                auto border = this->entity_->getComponent<Border>();
                this->collider_.x = (int) border->getTransform()->getPosition().getX();
                this->collider_.y = (int) border->getTransform()->getPosition().getY();
            } else {
